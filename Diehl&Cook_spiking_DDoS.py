...
xdata = np.load('x.npy')
ydata = np.load('y.npy')
trainingx = xdata[:180000]
testingx = xdata[180000:200000]
trainingy = ydata[:180000]
testingy = ydata[180000:200000]
previous_spike_count = np.zeros(n_e)
assignments = np.zeros(n_e)
input_numbers = [0] * num_examples
outputNumbers = np.zeros((num_examples, 2))
if not test_mode:
...
while j < (int(num_examples)):
    if test_mode:
        if use_testing_set:
            rates = trainingx[j] *  input_intensity
        else:
            rates = trainingx[j] *  input_intensity
    else:
        normalize_weights()
